...
global_step = tf.Variable(tf.zeros(0), name = 'global_step', trainable = False)
...
#d_sum=tf.histogram_summary('d',D_logit_real)
#d__sum=tf.histogram_summary('d',D_logit_fake)
#G_sum=tf.image_summary('G',G)

z_sum=tf.summary.histogram('z',z)
d_sum=tf.summary.histogram('d',D_logit_real)
d__sum=tf.summary.histogram('d_',D_logit_fake)
...
d_sum = tf.summary.merge([z_sum, d_sum, d_loss_real_sum, g_loss_sum])
